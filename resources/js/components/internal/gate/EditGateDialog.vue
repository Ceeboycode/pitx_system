...
function selectPicture(event: Event) {
    ...
    picturePreview.value = file
        ? URL.createObjectURL(file)
        : props.gate?.picture_url ?? null;
}
...
watch(open, (isOpen) => {
    if (isOpen && props.gate) {
        form.gate_name = props.gate.gate_name;
        form.status = props.gate.status;
        form.bays = props.gate.bays;
        form.location = props.gate.location.is_placeholder
            ? ''
            : props.gate.location.label;
        form.picture = null;
        form.clearErrors();
        ...
        <DialogContent class="">
            ...
            <form class="flex flex-col gap-y-2 px-6" @submit.prevent="submit">
                ...
                <DialogFooter class="gap-2">
                    <Button type="button" variant="ghost-outline" @click="open = false">Cancel</Button>
