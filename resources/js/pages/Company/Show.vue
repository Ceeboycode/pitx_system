                            ...
                            >
                                ...
                                <div class="min-w-0 space-y-2">
                                    ...
                                    <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
                                        <span v-if="doc.issued_at">Issued: <span class="font-medium text-foreground">{{ formatDate(doc.issued_at) }}</span></span>
                                        ...
                                        <span v-if="doc.uploader">Uploaded by: <span class="font-medium text-foreground">{{ doc.uploader.name }}</span></span>
                                        <span v-if="doc.created_at">on <span class="font-medium text-foreground">{{ formatDateTime(doc.created_at) }}</span></span>
                                    ...
                                    <div v-if="doc.remarks">
                                        <Popover>
                                            <PopoverTrigger as-child>
                                                <Button variant="outline" size="sm" class="h-7 rounded-lg border-slate-200 text-xs text-slate-600 hover:bg-slate-100">
                                            ...
                                            <PopoverContent align="start" class="w-80 rounded-xl">
                                                <div class="space-y-2">
                                                    ...
                                                    <p class="text-xs text-muted-foreground">{{ humanize(doc.doc_type) }} · {{ humanize(doc.status) }}</p>
                                                </div>
                                            </PopoverContent>
                                        </Popover>
                                    </div>
            ...
            <DialogContent class="flex max-h-[90vh] w-full max-w-4xl flex-col gap-0 overflow-hidden rounded-2xl p-0">
                ...
                <div class="relative flex-1 overflow-auto bg-muted/30">
                    <div v-if="previewDoc && isImage(previewDoc)" class="flex min-h-[50vh] items-center justify-center p-6">
                        <img :src="fileUrl(previewDoc)" :alt="previewDoc.original_name ?? previewDoc.doc_type" class="max-h-[70vh] max-w-full rounded-lg object-contain shadow-md" @error="(e) => ((e.target as HTMLImageElement).src = '')" />
                    </div>
        ...
        <AlertDialog v-model:open="confirmOpen">
            <AlertDialogContent class="rounded-2xl">
                ...
                <AlertDialogFooter>
                    ...
                    <AlertDialogAction class="rounded-lg bg-blue-700 text-white hover:bg-blue-800 border-0" :disabled="actionForm.processing" @click="runConfirmedAction">
        ...
        <AlertDialog v-model:open="bulkConfirmOpen">
            <AlertDialogContent class="rounded-2xl">
                ...
                <AlertDialogFooter>
                    ...
                    <AlertDialogAction class="rounded-lg bg-blue-700 text-white hover:bg-blue-800 border-0" :disabled="verifiedCount === 0" @click="runBulkDownload">
                        Continue
                    </AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
