...
import { RiAlertLine, RiArchive2Line, RiBusLine, RiDashboardHorizontalLine, RiFileListLine, RiHistoryLine, RiRoadMapLine, RiGroupLine } from 'vue-remix-icons';
...
const props = defineProps<{
    ...
    // Sent by UserController@show: whether the edited user's role gets each
    // permission-gated tab (see UserController::tabAccessFor).
    tabAccess: {
        vehicles: boolean;
        dispatches: boolean;
        employees: boolean;
...
// whose callback returns true, so the Vehicles/Employees/Dispatches tabs
// simply don't exist in the array (and don't render) when the backend's
// `tabAccess` says the role doesn't get them.
const tabs = computed(() =>
    [
        {
            value: 'overview',
        ...
        if (tab.value === 'employees') return props.tabAccess.employees;
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <PanelLayout>
            <LeadPanel>
                <ArchivedNotice v-if="props.isArchived" entity="user" />
                <LeadingCard
                    :title="user.name"
